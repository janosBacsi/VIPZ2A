...
//приймає єдиний параметр - вказівник на структуру Sbooks, який вказує на початок списку
void sortList(struct sBooks*);

//перевіряє чи відповідає стрічка формату імені/прізвища автора(лише літери, без пробілів)
...
//видаляє елемент за заданою адресою і звільняє пам'ять динамічно виділену для нього
//параметрами приймає: 1)вказівник на вказівник на адресу елементу, який треба видалити
//2)вказівник на вказівник на адресу елементу, який знаходиться перед елементом, який треба
...
//у випадку, якщо всі книги дорожчі за задану вартість, на екран виводиться повідомлення
...
//зберігає список в файл, адреса файлу вводиться користувачем під час виконання функції
//парамтером приймає вказівник на початок списку
void writeToFile(struct sBooks*);

//видаляє елемент за його індексом
//якщо елемента за взазаним індексом не існує, виводить на екран повідомлення про це 
...
//проводить пошук за прізвищем автора, яке задається користувачем в ході виконання функції
//якщо не знайдено елементів, які задовільняють умови, на екран виводиться відповідне повідомлення
//параметром приймає вказівник на вказівник початку списку
...
//знаходить дві книги з найбільшою кількістю сторінок і виводить їх на екран консолі
...
//надає можливість користувачеві редагувати елемент за заданим індексом, який задається під час виконання функції
//параметром приймає вказівник на початок списку
void editElementByIndex(struct sBooks*);

//зчитування дійсного додатнього значення з клавіатури, присутній захисть від некоректного вводу
//повертає введене користувачем значення
double readFoolProofDouble();

//зчитування цілого додатнього значення з клавіатури, присутній захисть від некоректного вводу
//повертає введене користувачем значення
